...
router.post('/create', verifyJWT, paymentRateLimiter, async (req, res) => {
  try {
    ...
    const sslSession = await paymentService.initSSLCommerz({
      transactionId,
    ...
    res.json({
      ...
      payment_url: sslSession.gatewayUrl,
...
router.post('/callback/sslcommerz/success', async (req, res) => {
  try {
    ...
    res.redirect(`${process.env.DASHBOARD_URL || 'http://localhost:3000'}/dashboard?payment=success`);

  } catch (error) {
    logger.error('SSLCommerz success processing failed:', error);
    res.redirect(`${process.env.DASHBOARD_URL || 'http://localhost:3000'}/billing?error=payment_verification_failed`);
  }
});
...
router.post('/callback/sslcommerz/fail', async (req, res) => {
  ...
  res.redirect(`${process.env.DASHBOARD_URL || 'http://localhost:3000'}/billing?error=payment_failed`);
});
